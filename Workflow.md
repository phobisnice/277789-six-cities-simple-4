...
#### Запустить json-server для моков

```bash
npm гun mock:server
```

Запуск сервера позволит обращаться за моковыми данными ./mocks/mock-server-data.json по адресу http://localhost:3123/api

### Сгенерировать моки 

```bash
npm run ts ./src/main.cli.ts -- --generate 10 ./mocks/mock-test.tsv http://localhost:3123/api
```
